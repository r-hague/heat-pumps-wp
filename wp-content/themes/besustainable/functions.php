<?php 
add_action( 'wp_enqueue_scripts', 'besustainable_enqueue_styles' );
function besustainable_enqueue_styles() {
	wp_enqueue_style( 'parent-style', get_template_directory_uri() . '/style.css' ); 
} 

function besustainable_remove_parent_fonts() {
	wp_dequeue_style( 'ecocoded-google-fonts' );
	wp_deregister_style( 'ecocoded-google-fonts' );
}
add_action( 'wp_enqueue_scripts', 'besustainable_remove_parent_fonts', 20 );
...
function besustainable_google_fonts() {
	wp_enqueue_style( 'besustainable-google-fonts', '//fonts.googleapis.com/css?family=Quicksand:400,500,600,700', false ); 
}
add_action( 'wp_enqueue_scripts', 'besustainable_google_fonts' );


function besustainable_more($more) {
	if ( is_admin() ) { return $more; }
	global $post;
	return '… <a href="'. get_permalink($post->ID) . '" class="excerpt-read-more-link">' . __('Continue reading' , 'besustainable') . '</a>';
}
add_filter('excerpt_more', 'besustainable_more', 999);




function besustainable_customize_register( $wp_customize ) {

	/* New Section */
	$wp_customize->add_section( 'besustainable_section', array(
		'title'      => __('BeSustainable Features','besustainable'),
		'priority'   => 1,
		'capability' => 'edit_theme_options',
	) );

	$wp_customize->add_setting( 'toggle_admin_byline', array(
		'default' => 0,
		'sanitize_callback' => 'besustainable_sanitize_checkbox',
	) );

	$wp_customize->add_control( 'toggle_admin_byline', array(
		'label'    => __( 'Remove author from post byline', 'besustainable' ),
		'section'  => 'besustainable_section',
		'priority' => 1,
		'settings' => 'toggle_admin_byline',
		'type'     => 'checkbox',
	) );

	$wp_customize->add_setting( 'besustainable_sidebar_bgcolor', array(
		'default'           => '#fff',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'postMessage',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'besustainable_sidebar_bgcolor', array(
		'label'       => __( 'Sidebar Background Color', 'besustainable' ),
		'section'     => 'besustainable_section',
		'priority'   => 1,
		'settings'    => 'besustainable_sidebar_bgcolor',
	) ) );
	$wp_customize->add_setting( 'besustainable_sidebar_headline_border', array(
		'default'           => '#000',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'postMessage',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'besustainable_sidebar_headline_border', array(
		'label'       => __( 'Sidebar Headline Border Bottom', 'besustainable' ),
		'section'     => 'besustainable_section',
		'priority'   => 1,
		'settings'    => 'besustainable_sidebar_headline_border',
	) ) );

	function besustainable_sanitize_checkbox( $input ) {
...
add_action( 'customize_register', 'besustainable_customize_register' );
...
if(! function_exists('besustainable_customizer_css_final_output' ) ):
	function besustainable_customizer_css_final_output(){
		...
		<style type="text/css">
			#secondary .widget-title:after { background: <?php echo esc_attr(get_theme_mod( 'besustainable_sidebar_headline_border')); ?>; }
			#secondary .swidgets-wrap { background: <?php echo esc_attr(get_theme_mod( 'besustainable_sidebar_bgcolor')); ?>; }
	...
	<?php }
	add_action( 'wp_head', 'besustainable_customizer_css_final_output' );
endif;


function besustainable_customize_preview_js() {
	wp_enqueue_script( 'besustainable-customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'besustainable_customize_preview_js' );
