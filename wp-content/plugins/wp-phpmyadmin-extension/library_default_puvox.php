...
{ 
	...
	public function vv($obj){ 
		...
		echo htmlentities( $this->br2nl( $content )) ;  try{ $file=basename(debug_backtrace()[1]['file']);  echo ($this->isCron() ? "<$file>" : '<span style="font-size:0.5em; margin:1px; padding:1px; background:pink;">'.$file.'</span>'); }catch(\Exception $e){} echo '</pre>'; 
	} 
	public function var_dump($obj){ $this->vv($obj); }
	public function var_dumpx($obj){ $this->vv($obj); exit; }
	...
	public function string_to_array($string, $divider='|', $allow_empty=false){ $res=( array_map('trim', explode($divider, $string) ) ); return $allow_empty? $res : array_filter($res); }
	...
	public function jsonPretty($array_or_txt) {  
		if(is_string($array_or_txt)) { 
			if ($this->is_JSON($array_or_txt)) 
	...
	function stripUnwantedTagsAndAttrs($html_str, $tags=null){
		$xml = new DOMDocument();
	...
	public function currentDatetime($time=''){
		return ( !empty($time) ? gmdate('Y-m-d h:i:s', $time) : gmdate('Y-m-d h:i:s') );
	}
	...
	public function file_get_contents($path, $createIfNotExistWithContent=null, $waitIfLocked=true)
	{
		if (!file_exists($path)){
	...
	public function array_value_empty($array, $key, $default=''){
	...
	public function array_makeKeyedBySubkey(&$array, $targetKey, $strict=true){
		$new_array=[];
	...
	public function array_unset_keys($array, $keysToUnset)
	{ 
	...
	public function array_children_unset_key($array, $keyToUnsetInChild, $onlyIfValueEqualsTo=null)
	{ 
		$newArray = [];
		foreach($array as $key=>$subArray) {
			if ( array_key_exists($keyToUnsetInChild, $subArray) ){
				if (is_null($onlyIfValueEqualsTo) || $subArray[$keyToUnsetInChild]===$onlyIfValueEqualsTo )
	...
	public function array_children_unset_keys($array, $keysToUnsetInChild, $onlyIfValueEqualsTo=null)
	{ 
		foreach($keysToUnsetInChild as $key) {
			$array = $this->array_children_unset_key($array, $key);
		}
		return $array;
	}

	public function array_children_with_keys($array, $keyNames, $resortKeysFromZero=false)
	{ 
		$newArray = [];
		if (!is_array($keyNames)) $keyNames=[$keyNames];
		
		foreach($array as $key=>$subArray) {
			$subar= $this->array_with_keys($subArray, $keyNames);
			$newArray[$key] = $subar;
		}
		return $newArray;
	}

	public function array_children_with_keyvalue($array, $keyName, $shouldBeEqualTo, $resortKeysFromZero=false)
	{ 
		$newArray = [];
		foreach($array as $key=>$subArray) {
			if ( array_key_exists($keyName, $subArray) && $subArray[$keyName]===$shouldBeEqualTo ){
	...
	public function array_children_replace_keyname($array, $keyName, $replaceWithKeyname)
	{ 
		//$newArray = [];
		//foreach($array as $key=>$subArray) {
		//	if ( $keyName===$key ){
	...
	public function array_children_replace_value($array, $keyName, $replaceWhatValue, $replaceWithValue)
	{ 
		$newArray = [];
		foreach($array as $key=>$subArray) {
	...
	public function array_without_keys($array, $keyToRemove){
		$new_arr = $array;
		//$keyIsArray= is_array($keyToRemove) ;
		//foreach($array as $key=>$Child)
		//{
		//	if ( ( $keyIsArray && !in_array($key,$keyToRemove) ) || (!$keyIsArray && $key!==$keyToRemove) )
		//		$new_arr[$key]=$Child;
		//}
		$keysToRemove=(is_array($keyToRemove)?$keyToRemove:[$keyToRemove]);
		foreach( $keysToRemove as $key )
			unset($new_arr[$key]);
		return $new_arr;
	}
	public function array_without_sub_keys($array, $keyToRemove, $include_child=true){
		$new_arr = [];
		foreach($array as $key1=>$subArray)
		{
			if ( is_array($subArray))
			{
				$new_arr[$key1]=$this->array_without_keys($subArray, $keyToRemove);
			}
			elseif($include_child){
				$new_arr[$key1]=$subArray;
			}
		}
		return $new_arr;
	}

	public function array_with_keys($array, $keyToRemain){
		$new_arr = $array;
		$keysToRemain= is_array($keyToRemain)?$keyToRemain:[$keyToRemain];
		foreach( $new_arr as $key=>$val )
		{
			if (!in_array($key,$keysToRemain))
				unset($new_arr[$key]);
		}	
		return $new_arr;
	}

	public function array_with_sub_keys($array, $keyToRemain, $include_child=true){
		$new_arr = [];
		foreach($array as $key1=>$subArray)
		{
			if ( is_array($subArray) )
			{
				$new_arr[$key1]=$this->array_with_keys($subArray, $keyToRemain);
			}
			elseif($include_child){
				$new_arr[$key1]=$subArray;
			}
	...
	public function array_with_sub_sub_keys($array, $keyToRemain, $include_child=true){
		$new_arr = [];
		foreach($array as $key1=>$subArray)
		{
			if ( is_array($subArray) )
			{
				$new_arr[$key1]=$this->array_with_sub_keys($subArray, $keyToRemain);
			}
			elseif($include_child){
				$new_arr[$key1]=$subArray;
			}
				...
				// if member is array/object
					...
					if ( is_array($value) ) 
					{
						if (!is_array($value_2) ) {
							...
							$newDiff = $this->array_diff_assoc_recursive($value, $value_2 );
							if( !empty($newDiff) )
							{
	...
	public function array_diff_recursive( $array_source, $array_toward )
	{
		return $this->array_diff_assoc_recursive($array_source, $array_toward);
	}
	
	public function array_difference( $array_source, $array_toward )
	{
		$diff = [
			'new'=> $this->array_diff_assoc_recursive($array_source, $array_toward),
			'old'=> $this->array_diff_assoc_recursive($array_toward, $array_source),
		];
		return $diff;
	}
	...
	public function ExceptionMessage($ex){
		return "[{$ex->getFile()}::{$ex->getLine()}] Message: {$ex->getMessage()}\r\n";
	}
	...
	public function sanitize($key){ return preg_replace( '/[^a-zA-Z0-9_\-]/', "_", trim($key) ); }  //same as wp
	public function sanitize_key($key, $use_dash=false ){ return preg_replace( '/[^a-z0-9_\-]/', ($use_dash===true ? "_": (is_string($use_dash) ? $use_dash: "") ), strtolower(trim($key) )); }  //same as wp  , 
	public function sanitize_key_($key, $use_dash=true ){ return $this->str_replace_recursive( "__","_",  $this->sanitize($key, $use_dash) ); }
	...
	public function sanitize_unicode($text, $replace_with=''){ 
	...
	public function remove_new_lines($text)
	{
		return str_replace(["\r","\n"],'', $text);
	}
	//
	public function removWhitespaces($input, $oneSpace=true){ 
		$what = $oneSpace ? ' ':'';
	...
	public function str_replace_recursive($value, $replace, $string) {
		$string = str_replace($value, $replace, $string);
		if (strpos($string, $value)!==false) {
			$string= $this->str_replace_recursive($value, $replace, $string);
		}
		return $string;
	}
	...
	// method 1
	public function remove_zero_from_end($input){
	...
	public function doubleNormal($input, $round_to=15, $use_sprintf=true){ 
		return (!is_float($input) && !is_numeric($input) ? $input : (float) $this->trim_zero_dot( $use_sprintf ? sprintf("%.{$round_to}f", $input) : number_format($input, $round_to) ) );	
	}
	public function number_format($input, $round_to=15){ 
		$ret =sprintf("%.{$round_to}f", $input) ;// number_format($input, $round_to);
		return $this->remove_zero_from_end($ret);
	}
	...
	public function telegram_message($array, $botid, $repeated_call=false){
		...
		$res = $this->get_remote_data_array( ['url'=>'https://api.telegram.org/bot'.$botid.'/sendMessage', 'post'=> $array] );  //'sendMessage?'.http_build_query($array, '');
		if (!$res->error){
			$answer = $res->response;
	...
	public function telegram_message_cached($array, $botid){
		...
		if ( $goneMS < $this->telegram_interval_ms ){
			usleep( ($this->telegram_interval_ms-$goneMS) *1000 );
		}
	...
	public function get_remote_data($url, $post_params=null, $request_options=[])	
	{
		...
		if (!$is_wp || $this->property('get_remote_data_ENABLE'))
		{
			if (is_callable('parent::get_remote_data')) { 
				return parent::get_remote_data($url, $post_params, $request_options);
			}
			else if (method_exists(get_parent_class(), 'get_remote_data')) { 
				$name= get_parent_class();
				$class = new $name();
				return $class->get_remote_data($url, $post_params, $request_options);
			}
			else if (method_exists($this, 'get_remote_data_')) {
				return $this->get_remote_data_($url, $post_params);
			}
		}
		else
		{ 
	...
	public function get_remote_data_array($arr, $repeated_call=false)
	{
		$url 			= $arr['url'];
		...
		$data = $this->get_remote_data($url, $post_params, $request_options ) ;
		if (empty($data))
		{
		...
		{
			usleep(100000);
			$res = $this->get_remote_data_array($arr, true);
		}
		return $res;
	}
	...
	public $async_methods_available = ['reactphp', 'amphp', 'spatie','parallel','fibers', 'swoole', 'exec', 'proc-open', 'popen', 'pcntl_fork', '_no_async_' ];
	...
	public function asyncFunctions($callbacksArray, $which_method='spatie', $blocking=true){
		$method = strtolower($which_method);
		if ( !in_array( $method, $this->async_methods_available) ) 
			throw new \Exception( "$method not supported, use from: ". implode(' | ',$this->async_methods_available) );
		...
		// check whether multiple functions array passed or signle
		if ( !is_array($callbacksArray[0]) )
		...
		{
			$func_name = 'asyncFunctions_helper_'.$method;
			call_user_func([$this,$func_name], $callbacksArray_NEW);
		}
		else{
	...
	// requires php 7.4+ parallel ZTS  
    public function asyncFunctions_helper_parallel($callbacksArray)
    {
		foreach( $callbacksArray as $callback_arg_pair){
			\parallel\run($callback = $callback_arg_pair[0], $args = $callback_arg_pair[1] ); 
		}
    } 
	// requires spatie's async  [ https://github.com/spatie/async/issues/120 ]
    public function asyncFunctions_helper_spatie($callbacksArray)
    {
		...
		$pool->wait(); 
    }  public function spatieSupported(){ return \Spatie\Async\Pool::isSupported(); } 

	// requires swoole extension
    public function asyncFunctions_helper_swoole($callbacksArray)
    {		
		\Swoole\Runtime::enableCoroutine(SWOOLE_HOOK_NATIVE_CURL);
		\Co\run(function() use ($callbacksArray)  {
			foreach( $callbacksArray as $callback_arg_pair){
				\go(function() use ($callback_arg_pair) {
					call_user_func_array($callback = $callback_arg_pair[0], $args = $callback_arg_pair[1]);
				});
			}
		});
    ...
    public function asyncFunctions_helper__no_async_($callbacksArray)
    {		
		$this->call_user_funcs($callbacksArray);
    } 
    public function call_user_funcs($callbacksArray)
    {
		foreach($callbacksArray as $callbackArray){
			$args = array_key_exists(1,$callbackArray) ? $callbackArray[1] : [];
	...
	public function IsSimpleType($var){
		return ( is_string($var) || is_numeric($var) || is_bool($var) );
	}

	public function ArgvGet($key='')
	{
		global $argv;
		$array = [];
		if (!empty($argv[1])) {
	...
	public function log_errors($path=false)
	{
		//htaccess:
...
class standard_wp_library extends standard_php_library  
{
	...
	{ 
		$array=$this->objectToArray( $this->get_table_my($tableName) );
	...
	*/
	...
	{  
		...
		//if (!empty($_POST)) { v($_POST); v(isset($_POST[$slug])); }
		return $result;
	}
	...
	*/
		...
		foreach($options_array as $key=>$block){
		  //try{
			...
			elseif( is_double($block['value']) )
				$line .= '<input name="'.$prefix.'['.$key.']" value="'. $this->array_value($currentvalues_array,$key) .'" placeholder="" style="width:70px;" />';
			elseif( is_integer($block['value']) )
				$line .= '<input name="'.$prefix.'['.$key.']" value="'. $this->array_value($currentvalues_array,$key) .'" placeholder="" style="width:70px;" />';
			elseif( is_string($block['value']) ){
				if($type=='textarea') 
					$line .= '<textarea name="'.$prefix.'['.$key.']">'. $this->array_value($currentvalues_array,$key) .'</textarea>'; 
				else                  
					$line .= '<input name="'.$prefix.'['.$key.']" value="'. $this->array_value($currentvalues_array,$key) .'" placeholder="" />'; 
			}
			elseif( is_array($block['value']) ) {
				$arr = $this->array_value($currentvalues_array,$key);
				$arr[$rand_id=$this->randomId('v')] = '';  
				foreach($arr as $bKey=>$bValue ){ 
					$is_random = $bKey==$rand_id;
					if( !empty($bValue) || $is_random ) {	
						if($type=='textarea') 
							$line .= '<textarea name="'.$prefix.'['.$key.']['.$bKey.']">'. $bValue .'</textarea>'; 
						else                  
							$line .= '<input name="'.$prefix.'['.$key.']['.$bKey.']" value="'. $bValue .'" placeholder="" />'; 
					}
				}
			}
		  ...
		  //} catch(\Exception $ex){ $line .= '<input name="'.$prefix.'['.$key.']" value="'. $ex->getMessage() .'" placeholder="" />';   }

		  // add line
		  $out .= '<tr class="trline"><td>'. $desc .'</td><td>'.$line.'</td> </tr>';
		}
		$out .= '</tbody></table>';
	  ...
	  public function options_default_parse( $args ){
		...
		if( $this->checkSubmission( $nonceKey, 'nonceactxwe3' ) ) {  
			foreach($options_array as $key=>$block){
				...
				elseif( is_string ($block['value']) )   $res[$key] = sanitize_textarea_field($_POST[$prefix][$key]); 
				elseif( is_array  ($block['value']) )	$res[$key] = $this->array_map_recursive( 'sanitize_textarea_field', $_POST[$prefix][$key] );
			}
		}
		return $res;
	  }
	...
	public function append_to_transient($transientName, $key=null, $valueToAddInArray=null, $seconds= 8640000){
		$current_arr= $this->get_transient($transientName, []);
		if ($key) $current_arr[$key] = $valueToAddInArray;
		else $current_arr[] = $valueToAddInArray;
		return set_transient($transientName,$current_arr, $seconds);
	}
	public function check_timer_remains($uniqueActionId, $seconds=9999999999, $reset=false ){
		$remains = 0;
		if ( $seconds>0 )
		{
			...
			if( $last_time >0  ) { 
				$remains = ($last_time + $seconds) - time();
			}
			else{
				$this->set_transient($transientName, time(), $seconds);
			}
		}
		return $remains;
	}
	...
	public function create_table_my($table_name, $array){
		global $wpdb;
	...
	public function update_or_insert($tablename, $NewArray, $WhereArray=[]){   	
		...
		if(!empty($WhereArray)){
			...
			$o=''; $i=1; foreach ($WhereArray as $key=>$value){ $o .= $wpdb->prepare( sanitize_key($key) . " = '%s'", $value); if ($i != count($WhereArray)) { $o .=' AND '; $i++;}  }
			$sql = ("SELECT * FROM `".sanitize_key($tablename)."` WHERE $o");
	...
	public function db_get_results($table_name='tablename', $what='', $key='', $value=''){
		global $wpdb;
		if (empty($key) )
			$sql = "SELECT ". (empty($what) ? '*':esc_sql($what))." FROM ".sanitize_key($table_name);
		else
			$sql = $wpdb->prepare( "SELECT ". (empty($what) ? '*':esc_sql($what))." FROM WHERE `".sanitize_key($key)."` = '%s'", sanitize_key($table_name),  $value);
	...
	public function db_get_row($table_name='tablename', $key='', $value='', $type='object'){
		...
		return ($type==='object' ? $res : $this->objectToArray($res));
	} 
	...
	public function db_set_datacache($key, $data, $time=false)
	{    
		$time = $time ?: time();
	...
	public function db_get_datacache($key, $decode='array')
	{    
		$res = $this->db_get_var( $this->db_alldata_tablename(), 'option_name', $key, 'text' );
		if ($decode){
			if (empty($res))
				$res='{}';
			try{
				return json_decode($res, ($decode==='array'));
			}
			catch(\Excetpion $ex){
				return $res;
			}
	...
	public function db_append_datacache($key, $data, $time=false)
	{    
		$existing=$this->db_get_datacache($key);
		$time = $time ?: time();
	...
	{	
		...
		try{
			...
			$args_str=''; foreach($last_func['args'] as $i=>$e) { $args_str .= "\r\n[$i] ---&gt; ". $this->charsFromStart( print_r($e,true), $trim_chars) ;} 
...
class default_plugin 
{
	...
	public function update_opts($opts=null, $permission_check=true){
		$opts = ( $opts!==null && $opts!==false ? $opts : $this->opts);
		$this->opts = $opts;
		$res  = ($permission_check && (did_action('init') && !current_user_can('manage_options')) ) ? 222 : $this->update_option_CHOSEN($this->slug, $this->opts ) ;
		return $res;
	}
